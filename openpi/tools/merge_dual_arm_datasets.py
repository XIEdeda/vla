    ...
    for i, (old_folder, old_index, new_index) in enumerate(episode_mapping):
        ...
        if source_path:
            try:
                ...
                # print("602===============",table.schema.metadata)
                # 检查是否需要填充维度 - 为不同特征类型使用不同的最大维度
                # 为状态向量填充
                if "state" in table.column_names:
                    # 检查第一个非空值 (Check first non-null value)
                    state_column = table.column("state")
                    for i in range(len(state_column)):
                        state_val = state_column[i].as_py()
                        if state_val is not None and isinstance(state_val, (list, np.ndarray)):
                            current_dim = len(state_val)
                            if current_dim < state_max_dim:
                                print(f"填充状态向量从 {current_dim} 维到 {state_max_dim} 维")
                                # 创建新的填充后的数组
                                padded_arrays = []
                                for j in range(len(state_column)):
                                    val = state_column[j].as_py()
                                    if val is not None and isinstance(val, (list, np.ndarray)):
                                        padded = np.pad(val, (0, state_max_dim - len(val)), "constant").tolist()
                                        padded_arrays.append(padded)
                                    else:
                                        padded_arrays.append(val)
                                # 替换原列
                                new_state_col = pa.array(padded_arrays)
                ...
                # 为动作向量填充
                if "actions" in table.column_names:
                    action_column = table.column("actions")
                    # 完整遍历整个列来检测维度
                    for i in range(len(action_column)):
                        action_val = action_column[i].as_py()
                        if action_val is not None and isinstance(action_val, (list, np.ndarray)):
                            current_dim = len(action_val)
                            if current_dim < action_max_dim:
                                print(f"填充动作向量从 {current_dim} 维到 {action_max_dim} 维")
                                padded_arrays = []
                                for j in range(len(action_column)):
                                    val = action_column[j].as_py()
                                    if val is not None and isinstance(val, (list, np.ndarray)):
                                        padded = np.pad(val, (0, action_max_dim - len(val)), "constant").tolist()
                                        padded_arrays.append(padded)
                                    else:
                                        padded_arrays.append(val)
                                new_action_col = pa.array(padded_arrays)
                                table = table.set_column(
                                    table.column_names.index("actions"),
                                    "actions",
                                    new_action_col
                                )
                            break
                ...
                if "episode_index" in table.column_names:
                    old_episode_idx = table.column("episode_index")[0].as_py()
                    print(f"更新episode_index从 {old_episode_idx} 到 {new_index}")
                    new_episode_col = pa.array([new_index] * len(table))
                    table = table.set_column(
                ...
                if "index" in table.column_names:
                    if episode_to_frame_index and new_index in episode_to_frame_index:
                        ...
                        print(f"更新index列，起始值: {first_index}")
                    else:
                        first_index = new_index * len(table)
                        print(f"更新index列，起始值: {first_index}")

                    new_index_col = pa.array([first_index + i for i in range(len(table))])
                    table = table.set_column(
                 ...
                 # 更新task_index列
                if "task_index" in table.column_names and folder_task_mapping and old_folder in folder_task_mapping:
                    old_task_idx = table.column("task_index")[0].as_py()
                    if old_task_idx in folder_task_mapping[old_folder]:
                        new_task_index = folder_task_mapping[old_folder][old_task_idx]
                        print(f"更新task_index从 {old_task_idx} 到 {new_task_index}")
                        new_task_col = pa.array([new_task_index] * len(table))
                        table = table.set_column(
                        ...
                        print(f"警告: 找不到task_index {old_task_idx}的映射关系")
                    ...
                    if file.endswith(".parquet") and f"episode_{old_index:06d}" in file:
                        try:
                            ...
                            if "observation.state" in table.column_names:
                                state_column = table.column("observation.state")
                                for i in range(len(state_column)):
                                    state_val = state_column[i].as_py()
                                    if state_val is not None and isinstance(state_val, (list, np.ndarray)):
                                        current_dim = len(state_val)
                ...
                for file in files:
                    ...
                    if file.endswith(".parquet"):
                        try:
                            ...
                            if "state" in table.column_names:
                                state_column = table.column("state")
                                for i in range(len(state_column)):  # 遍历整个列
                                    state_val = state_column[i].as_py()
                                    if state_val is not None and isinstance(state_val, (list, np.ndarray)):
                                        folder_state_dim = len(state_val)
                                        print(f"Detected {folder_state_dim} dimensions for observation.state in {folder}")
                                        break  # 找到第一个有效值后跳出
                            
                            # 检查动作向量维度
                            if "actions" in table.column_names:
                                action_column = table.column("actions")
                                for i in range(len(action_column)):  # 遍历整个列
                                    action_val = action_column[i].as_py()
                                    if action_val is not None and isinstance(action_val, (list, np.ndarray)):
            ...
            for episode in episodes:
                ...
                if old_index in stats_map:
                    ...
                    if "stats" in stats:
                        ...
                        if "state" in stats["stats"] and folder_state_dimensions[folder] < state_max_dim:
                            for stat_type in ["mean", "std", "max", "min"]:
                                if stat_type in stats["stats"]["state"]:
                                    values = stats["stats"]["state"][stat_type]
                                    if isinstance(values, list) and len(values) < state_max_dim:
                                        padded = values + [0.0] * (state_max_dim - len(values))
                                        stats["stats"]["state"][stat_type] = padded
                        
                        # 分别处理action的统计数据
                        if "actions" in stats["stats"] and folder_action_dimensions[folder] < action_max_dim:
                            for stat_type in ["mean", "std", "max", "min"]:
                                if stat_type in stats["stats"]["actions"]:
                                    values = stats["stats"]["actions"][stat_type]
                                    if isinstance(values, list) and len(values) < action_max_dim:
                                        padded = values + [0.0] * (action_max_dim - len(values))
                                        stats["stats"]["actions"][stat_type] = padded
...
if __name__ == "__main__":
    ...
    parser.add_argument("--state_max_dim", type=int, default=16, help="Maximum state vector dimension (default: 32)")
    parser.add_argument("--action_max_dim", type=int, default=16, help="Maximum action vector dimension (default: 32)")
    ...
    # merge_datasets(
    # ["/home/xman/wyx/keenon_lerobot/test_data/test/dual_arm_test_1","/home/xman/wyx/keenon_lerobot/test_data/test/dual_arm_test_2"], 
    # "/home/xman/wyx/keenon_lerobot/test_data/merge", 
    # state_max_dim=16, 
    # action_max_dim=16,
    #  default_fps=20
    # )