...
// ==============================
// 进程内 mcap 写入队列
// 回调线程只负责把轻量 lambda (仅捕获 shared_ptr) 推入队列，
// 专用写线程串行消费，序列化 + 写盘完全在回调线程之外完成。
// Writer 仅被写线程访问，无需外部 write 锁。
// ==============================
...
public:
    ...
    /// 回调线程调用：推入写任务（仅拷贝 lambda，不拷贝图像数据）
    ...
    /// 停止接受新任务，排空队列后 join 写线程（end_episode 调用）
    void stop() {
...
private:
    ...
    /// use_mcap=true 时订阅 sensor_msgs/msg/CompressedImage（话题见 ROS2_CAMERA_TOPIC_MAP * _compressed）
...
public:
    ...
    /// 推送给前端的展示文案：motor_error_status 为最近一次 topic 解析结果；超时时视为未获取
    std::string get_motor_error_status_effective() const;

    /// 由 motor_errors 回调更新。读展示请用 get_motor_error_status_effective()（会判断超时未更新）
...
private:
    ...
    /// 电机错误回调: error_codes[0..13]电机故障码,[14]右臂/[15]左臂虚拟墙预警,[16]右臂/[17]左臂掉电(0正常,非0异常/预警)
    ...
    /// 每次 _motor_errors_callback 成功解析后更新；用于“一段时间无新消息”判定
