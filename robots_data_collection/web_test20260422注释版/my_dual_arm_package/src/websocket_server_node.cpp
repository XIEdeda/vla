...
/// 解析 WebSocket 帧的 mid：避免客户端发 "mid": null 触发 nlohmann::json::type_error.302
...
// Samba 上传配置: 优先从 /etc/orin_samba.conf 读取,失败则用下方默认值
...
// ----- /etc/robot_config.json：启动时上报采集端 URL（AES-128-CBC/PKCS5 + SHA256 签名）-----
// 配置项 robotIp（或兼容旧字段 url）、robotSn、tenantId、apiBaseUrl；密文明文 url 与 json 中字符串完全一致，不改写。
// 请求体默认 params、timestamp、signature（可用 encryptField 改为 param）
// aesKey/aesIv 不配时使用下方默认（可被 json 覆盖）
...
/// 32 位十六进制（解析为 16 字节）或与 Java AES 一致：各 16 个 ASCII 字符直应当作 16 字节 key / iv
...
/// 从配置中的 url 推导 API 根地址；可选 apiBaseUrl 覆盖。未带 scheme 时默认 http。
...
/// 若存在 /etc/robot_config.json 则读取并 POST 上报；文件不存在则忽略。
void maybe_report_robot_url_from_config() {
    ...
    // 现网为 params；若需 param，在 robot_config.json 设 "encryptField": "param"
...
static const std::string GUODI_DATASET_ROOT = "/mnt/keenon_baidu_dataset";
static std::mutex g_samba_upload_mutex;  ///< 串行化Samba上传

// ====== GET_AGENT_ID / robot_status.json 状态上报相关 ======
static std::mutex g_robot_status_mutex;
static std::string g_robot_status_agent_id;  ///< 当前生效的 agent_id
static std::shared_ptr<net::steady_timer> g_robot_status_timer;  ///< 每10秒更新 robot_status.json 的定时器


// ----- 前端 START_ARM_CONTROL / STOP_ARM_CONTROL：启动/停止 sc_ros2 remote_control_data -----
static std::mutex g_remote_control_mutex;
static pid_t g_remote_control_pid = -1;
/// 默认与手动在 ~/sc_ros 下执行一致；可通过指令 data.workdir / data.command 覆盖
...
/// 在独立会话中启动 shell 子进程（setsid + 可选 chdir），便于对进程组发 SIGINT 结束（等同 Ctrl+C）
...
/// 生成 ISO8601 时间戳 (如 2025-11-20T10:30:00.000Z)
...
/// 将 meta_info 的 create_time ("2026-02-27 16:01:54 +08:00") 转为 ISO8601 UTC
/// 注意：根据反馈，字符串中的时间部分已是 UTC 时间，忽略时区后缀
...
static std::string compute_end_time_iso8601(const std::string& start_time_iso8601, int duration_sec) {
    ...
    if (sscanf(start_time_iso8601.c_str(), "%d-%d-%dT%d:%d:%d", &y, &mo, &d, &h, &mi, &s) >= 6) {
        ...
        tm.tm_hour = h;
        tm.tm_min = mi;
        tm.tm_sec = s + duration_sec; // 直接相加，timegm 会自动处理溢出进位
        tm.tm_isdst = 0;

#if defined(__linux__) || defined(__GLIBC__)
        // timegm 将 tm 视为 UTC 时间，并处理秒数溢出（如 70秒 会自动变成 1分10秒）
...
/// 根据机械臂/相机状态计算 robot_status 的 status/errorCode/errorMsg
/// out_status: errorCode 0 为 running；1/2/3 为 error。
/// errorCode: 0 全正常，或（相机正常且机械臂仅虚拟墙预警，errorMsg 保持空）
///           | 1 相机正常，机械臂异常（含未获取 motor_errors、须停采、其他非仅虚拟墙）| 2 机械臂正常，相机异常 | 3 相机与机械臂均异常
static void compute_robot_status_fields(std::string& out_status, int& out_error_code, std::string& out_error_msg) {
    out_status = "running";
    out_error_code = 0;
    out_error_msg = "";
    try {
        // 优先 get_current_instance 避免打断预览；实例为空(如 RECOVER 后)则 get_instance 创建以便读取硬件状态
        ...
        const bool motor_vwall_only =
            !motor_must_stop && motor_status.find("虚拟墙预警") == 0;  // 与 ros2 motor_error_status 前缀一致

        // 未获取/其他机械臂问题与「仅虚拟墙」不同：与相机正常组合时走 errorCode 1
...
/// 将 robot_status.json 写入 Samba 共享目录 /data/agent_id/
static void write_robot_status_to_samba(const std::string& agent_id) {
    try {
        ...
        // 独立于 g_samba_upload_mutex，不与 STOP_AND_SUB / UPLOAD_TO_SAMBA / WRITE_SUB_TASK_JSON 争抢锁
...
static void schedule_robot_status_update(const Executor& exec) {
    ...
    g_robot_status_timer->async_wait([](const boost::system::error_code& ec) {
        ...
        if (!aid.empty()) {
            // 放到独立线程执行，不阻塞 io_context，不影响采集/帧推送/WebSocket
...
class Session : public std::enable_shared_from_this<Session> {
    ...
    void cleanup_on_disconnect() {
        ...
        stop_robot_status_timer();  // 断开时停止 robot_status.json 的定时写入
        if (processor_) {
...
public:
    ...
    void handle_command(const std::string& cmd, int mid, const json& data) {
        ...
        // ====== GET_AGENT_ID: 设置 agent_id 并启动 robot_status.json 上报 ======
        ...
        // ====== GET_ROBOT_STATUS: 获取相机和机械臂状态 ======
            ...
            try {
                auto proc = DoubleArmRGBDDataProcessor::get_current_instance();
                if (!proc) proc = DoubleArmRGBDDataProcessor::get_instance();  // RECOVER 后实例已销毁，需重建以便读取状态
                if (proc) {
                    motor_status = proc->arm_interface ? proc->arm_interface->get_motor_error_status_effective() : "未知";
        ...
        // ====== START_TELEMETRY: 启动数据采集 ======
            ...
            const fs::path default_root = "/mnt/keenon_baidu_dataset";
        ...
        // ====== STOP_AND_SUB: 停止采集落盘 + 上传到 Samba(PC) ======
            ...
            try {
                ...
                // 2. 校验并上传到 Samba(参考 UPLOAD_TO_SAMBA)
                if (data_folder_.empty()) {
                ...
                // 3. 创建 Samba 上的多级目录(agent_id, agent_id/task_id, agent_id/task_id/1 等)
                ...
                std::string cd_chain;  // 累积 cd 命令以进入各级目录
        ...
        // ====== DISCARD_BOTH_DATA: 丢弃本机+Samba两边的采集数据 ======
            ...
            try {
                ...
                if (src_str.size() > root_str.size() && src_str.find(root_str) == 0 && src_str[root_str.size()] == '/') {
                    ...
                    if (rel_path.find("..") == std::string::npos && !rel_path.empty()) {
                        ...
                        // 0. 从上一条采集路径解析 agent_id/task_id/sub_number，删除 sub_task.json 中对应条目
                        std::vector<std::string> parts;
                        for (size_t i = 0; i < rel_path.size(); ) {
                            size_t j = rel_path.find('/', i);
                            if (j == std::string::npos) {
                        ...
                        // 2. 深度清理 parameters (二级结构: parameters/camera, parameters/hardware)
        ...
        // ====== UPLOAD_TO_SAMBA: 将 TASK_ID 文件夹上传到Samba共享目录 ======
        } else if (cmd == "UPLOAD_TO_SAMBA") {
            try {
                std::lock_guard<std::mutex> lock(g_samba_upload_mutex);
                // 前端传入 agent_id 和 task_id, 数据路径为 /mnt/keenon_baidu_dataset/{AGENT_ID}/{TASK_ID}
                std::string agent_id = data.contains("agent_id") ? data["agent_id"].get<std::string>() : "";
                ...
                // 安全校验: 路径必须在 /mnt/keenon_baidu_dataset 下
                fs::path canonical_root = fs::path(GUODI_DATASET_ROOT).lexically_normal();
                fs::path canonical_source = source_path.lexically_normal();
                std::string src_str = canonical_source.string();
                std::string root_str = canonical_root.string();
                ...
                // smbclient 直接递归上传, 目标路径: /data/agent_id/task_id/
                std::string smb_base = "smbclient \"//" + g_samba_host + "/" + g_samba_share
                    + "\" -U " + g_samba_user + "%" + g_samba_pass;

                // 1. 创建 agent_id、agent_id/task_id 目录(若已存在则忽略)
                std::system((smb_base + " -c \"mkdir " + agent_id + "\" 2>/dev/null").c_str());
                std::system((smb_base + " -c \"cd " + agent_id + "; mkdir " + task_id + "\" 2>/dev/null").c_str());

                // 2. 进入 agent_id/task_id 并递归上传 task_id 文件夹内容
        ...
        // ====== LIST_FILES: 列出文件夹内容 ======
            ...
            std::string path = data.contains("path") ? data["path"].get<std::string>() : "/mnt/keenon_baidu_dataset";
        ...
        // ====== DOWNLOAD_FILE: 文件下载服务 ======
            ...
            try {
                if (file_path.empty()) {
                    ...
                    // 因为基础路径是"/"，所以URL路径应该是从根开始的完整路径
                    std::string url_path = file_path_obj.relative_path().string();
                    // 确保路径不以/开头，因为/files/已经包含分隔符处理
                    if (!url_path.empty() && url_path[0] == '/') {
                        url_path = url_path.substr(1);
                    }
                    // 使用WebSocket服务器的监听地址（自动获取eth网卡IP）
        ...
        // ====== VERIFY_DATA: 数据校验 ======
            ...
            try {
                // 优先从请求中获取 data_folder，如果没有则使用 Session 中保存的 data_folder_
                ...
                // 如果 meta_info.json 存在（说明采集已结束并落盘），优先从文件中读取真实统计值
                ...
                if (fs::exists(meta_path)) {
                    try {
                        ...
                        if (meta.contains("arm_frame_details")) {
                            // 将文件中的 {"left_arm": 380, "right_arm": 380} 映射回 {"left": 380, "right": 380}
        ...
        // ====== WRITE_SUB_TASK_JSON: 写入 sub_task.json 到 PC (Samba) ======
        // 前置: START_TELEMETRY 已传入 data_folder(如 agent_id/task_id/1)，Session 保存了 data_folder_
            ...
            try {
                ...
                // 从 data_folder_ 解析 agent_id 和 task_id (路径格式: .../agent_id/task_id/xxx)
                fs::path canonical_root = fs::path(GUODI_DATASET_ROOT).lexically_normal();
                ...
                if (base_str.size() > root_str.size() && base_str.compare(0, root_str.size(), root_str) == 0) {
                    ...
                    if (parts.size() >= 3) {
                        agent_id = parts[0];
                        task_id = parts[1];
                        sub_number = parts[2];  // 如 "1"，用作 subNumber
                    } else if (parts.size() >= 2) {
                        agent_id = parts[0];
                        task_id = parts[1];
                        sub_number = task_id;   // 仅两段时退化为 task_id
                    }
                }
                if (agent_id.empty() || task_id.empty()) {
                ...
                // file_Size: 优先从请求获取(VERIFY_DATA 返回的 folder_size_mb)，否则从目录计算
                ...
                // 读取现有 sub_task.json(若存在)，合并: 同 subNumber 则覆盖，否则追加
        ...
        // ====== RECOLLECT_TASK: 重新采集 - 仅删除数据目录，重置 sub_task.json，其他文件不删 ======
        ...
        // ====== CANCEL_COLLECTION: 取消采集 - 删除整个 task 文件夹（机器端+PC端） ======
...

                {
                    ...
                    for (const std::string& sub : subdir_set) {
                        ...
                        if (sub == "task_info") {
                            ...
                            // 清理数据子目录 (camera, parameters, record)
                            std::vector<std::string> cams = {"hand_left", "hand_right", "head"};
                            std::vector<std::string> types = {"color", "depth"};
                            for (const auto& cam : cams) {
                                for (const auto& type : types) {
                    ...
                    // 最后清理 task_id 根目录下的残留文件并删除 task_id 文件夹本身
        ...
        // ====== START_ARM_CONTROL: 启动 sc_ros2 remote_control_data（等价 taskset + ros2 run）======
        ...
        // ====== STOP_ARM_CONTROL: 结束 remote_control（对进程组 SIGINT，等同 Ctrl+C）======
