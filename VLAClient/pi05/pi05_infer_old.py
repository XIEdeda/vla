...
class RosOperator(Node):
    ...
    def control_arm_chunk_freq(
        self, actions: np.ndarray, mode: int = 0, frequency: float = 10
    ):
...
def main():
    ...
    try:
        while rclpy.ok():
            ...
            # sub_actions = process_actions(actions, mode="ema", value=0.1)
            # sub_actions = remove_small_change_actions(sub_actions)
            actions[:, 7] = np.clip((actions[:, 7] * 100 - 55) * 70 / 15, 0, 70)
            ...
            tic = time.time()
            ros_op.control_arm_chunk_freq(sub_actions, mode=0, frequency=10)
            toc = time.time()
