...
#define RIGHT_GRIPPER_FORCE 0.32
#define INTERPOLATION_NUM_POINTS 1
...
void right_robot_arm_control_thread() {
    RCLCPP_INFO(rclcpp::get_logger("RobotArmControl"), "Right robot arm control thread started with interpolation");
    
    std::vector<double> current_angles(7, 0.0);
    std::vector<double> target_angles(7, 0.0);
    bool has_target = false;
    bool is_moving = false;
    
    // 获取初始位置
    std::vector<double> initial_angles = getJointAngle_r();
    for(int i = 0; i < 7; i++) {
        current_angles[i] = initial_angles[i] / 180.0 * M_PI;
    }
    ...
    while (robot_arm_thread_running) {
        // 检查是否有新目标
        {
            ...
            if (!right_robot_arm_queue.empty()) {
                target_angles = right_robot_arm_queue.front();
                right_robot_arm_queue.pop();
                has_target = true;
                is_moving = false;  // 重置运动状态，准备新插值
                
                RCLCPP_INFO(rclcpp::get_logger("RobotArmControl"), 
                           "New target received, will interpolate from current position");
            }
        }
        ...
        if (has_target) {
            if (!is_moving) {
                ...
                // 生成10个插值点
                const int num_points = INTERPOLATION_NUM_POINTS;
                std::vector<std::vector<double>> interpolated_points;
                
                for (int step = 1; step <= num_points; step++) {
                    double t = (double)step / num_points;  // 插值系数 0.1, 0.2, ..., 1.0
                    std::vector<double> mid_angle(7);
                    for (int j = 0; j < 7; j++) {
                        mid_angle[j] = current_angles[j] + (target_angles[j] - current_angles[j]) * t;
                    }
                    interpolated_points.push_back(mid_angle);
                }
                
                // 依次执行插值点
                for (const auto& point : interpolated_points) {
                    // 在执行每个插值点之前，检查是否有新数据到达
                    ...
                    if (new_data_arrived) {
                        // 新数据到达，中断当前插值
                        RCLCPP_INFO(rclcpp::get_logger("RobotArmControl"), 
                                   "New data arrived, interrupting current interpolation");
                        is_moving = false;
                        break;
                    }
                    
                    // 执行当前插值点
                    moveJ_r(point, JOINT_SPEED);
                    std::this_thread::sleep_for(std::chrono::milliseconds(10));
                }
                
                // 如果插值正常完成（没有被中断），更新当前位置
                if (is_moving) {
                    current_angles = target_angles;
                    has_target = false;
                    is_moving = false;
                    RCLCPP_INFO(rclcpp::get_logger("RobotArmControl"), 
                               "Interpolation completed, reached target");
                } else {
                    // 被中断了，不更新 current_angles，保持当前位置
                    // 下一次循环会用新的 target 从当前位置插值
                    RCLCPP_INFO(rclcpp::get_logger("RobotArmControl"), 
                               "Interpolation interrupted, current position preserved");
                }
            }
        } else {
            // 没有目标时，短暂休眠避免空转
...
void left_robot_arm_control_thread() {
    RCLCPP_INFO(rclcpp::get_logger("RobotArmControl"), "Left robot arm control thread started with interpolation");
    ...
    // 获取初始位置
    std::vector<double> initial_angles = getJointAngle_l();
    for(int i = 0; i < 7; i++) {
        current_angles[i] = initial_angles[i] / 180.0 * M_PI;
    }
    ...
    while (robot_arm_thread_running) {
        // 检查是否有新目标
        {
            ...
            if (!left_robot_arm_queue.empty()) {
                target_angles = left_robot_arm_queue.front();
...
void gripper_right_control_thread() {
    ...
    while (gripper_right_thread_running) {
        ...
        controlRightClamp(command_gripper_width, RIGHT_GRIPPER_FORCE);
...
void gripper_left_control_thread() {
    ...
    while (gripper_left_thread_running) {
        ...
        controlLeftClamp(command_gripper_width, LEFT_GRIPPER_FORCE);
...
int main(int argc, char **argv) {
    ...
    int retr = moveJ_r(zero_angle_r, JOINT_INIT_SPEED);
    int retl = moveJ_l(zero_angle_l, JOINT_INIT_SPEED);
